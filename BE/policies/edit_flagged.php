...
<body style= "background: #008374; margin-bottom: 50px;">
  ...
  <header id="header" class="header d-flex align-items-center">
    ...
    <div class="container-fluid container-xl d-flex align-items-center justify-content-between">
      ...
      <nav id="navbar" class="navbar">
        <ul>
          <li><a href= "flagged.php"> Go Back</a></li>
        </ul>
  ...
  <div>
    <form method="POST" action="edit_flagged.php" ID="login-form" style=
    "width: 50%;
    background: white;
    margin: 10px;
    padding: 19px;
    ...
    margin-left: 25%;">
        ...
        <input type="text" name="Comments" min="0" step="0.01"  class = "ictextfield">
        <br>
        <br>
        <input type="hidden" name="policy_number" value="<?php echo $policy_number; ?>">
        <button type="submit" class= "mbutton">Update Policy</button>
    ...
    <?php
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
          ...
          $result = UpdatePolicy($policy_number, $effective_date, $expire_date, $total_amount,$Vehicle_Type, $Number_Of_Units, $Policy_Type, $Price_Per_Unit, $percentage, $Total_Perm, $Currency, $Coverage_Per_Unit, $Third_Party_Coverage, $Coverage, $Manufacturer,$Comments);

          if ($result) {
            // Redirect user to flagged policies pages again
            echo '<script>window.location.href = "flagged.php";</script>';
          } else {
            // Display error message
            echo "<script>alert('Error Updating policy')</script>";
          }
        }
