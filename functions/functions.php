...
function addpolicydetails($policy_ID, $Vehicle_Type, $Number_Of_Units, $Policy_Type, $Price_Per_Unit, $percentage, $Total_Perm, $Currency, $Coverage_Per_Unit, $Third_Party_Coverage, $Coverage, $Manufacturer){
       // Connect to the database
  $conn = mysqli_connect("localhost", "root", "root", "ic1");
  ...
  // Prepare the SQL statement
  $sql = "INSERT INTO policy_details (ID, Vehicle_Type, Number_Of_Units, Policy_Type, Price_Per_Unit, Percentage_Of_Permium, Total_Perm, Currency, Coverage_Per_Unit, Third_Party_Coverage, Coverage, Manufacturer)) 
  VALUES ('$policy_ID', '$Vehicle_Type', '$Number_Of_Units', '$Policy_Type', '$Price_Per_Unit', '$percentage', '$Total_Perm', '$Currency', '$Coverage_Per_Unit', '$Third_Party_Coverage', '$Coverage', '$Manufacturer')";
...
function flag($policy_number){
    ...
    // Prepare the SQL statement
      $sql = "UPDATE policies SET Status = 'Flagged' WHERE PolicyNumber = '$policy_number'";
    // Execute the SQL statement
      if (mysqli_query($conn, $sql)) {
          $result = true;
      } else {
